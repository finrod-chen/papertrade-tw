...
Docker 部署（image 由 GitHub Actions 自動建置並推到 Docker Hub，支援 amd64 / arm64）：

```bash
docker pull s02260441/papertrade-tw:latest
docker run -p 5000:5000 -e FINMIND_TOKEN=你的token \
  -v ./logs:/app/logs -v ./data:/app/data s02260441/papertrade-tw:latest
```
