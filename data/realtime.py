...
# 上櫃股票代碼清單（不完整，遇到 .TW 抓不到時會自動試 .TWO）
_OTC_IDS = {
    "6505", "3008", "2727", "6415", "3037",  # 台灣之星、矽創等
}


def _ticker(stock_id: str) -> str:
    suffix = ".TWO" if stock_id in _OTC_IDS else ".TW"
...
def get_intraday_ohlcv(stock_id: str, interval: str = "5m") -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        print(f"[realtime] {stock_id} 分 K 失敗: {e}")
        return None
